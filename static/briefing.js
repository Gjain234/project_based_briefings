...
function setupEventListeners() {
  ...
  document.getElementById('projectSelectionCloseBtn').addEventListener('click', closeProjectSelectionModal);
  document.getElementById('projectSelectionCancelBtn').addEventListener('click', closeProjectSelectionModal);
  document.getElementById('projectSelectionModal').addEventListener('click', (e) => {
    if (e.target.id === 'projectSelectionModal') closeProjectSelectionModal();
  });
...
function buildBriefingRequest(customPrompt = null, selectedProjectIds = null, options = {}) {
  const country = document.getElementById('countrySelect').value;
  const mode = document.getElementById('briefingMode').value;
  const nParagraphs = parseInt(document.getElementById('nParagraphs').value, 10);
  const forceRegenerate = document.getElementById('forceRegenerate').checked;
  const regenerateFinalOnly = options.regenerateFinalOnly === true;
  ...
  return {
    ...
    force_regenerate: regenerateFinalOnly ? false : forceRegenerate,
    regenerate_final_only: regenerateFinalOnly,
...
function validateCustomCategories(customCategories) {
  ...
  const suspiciousPatterns = [
    /(^|\s)(you are|write exactly|for each paragraph|citation rules|important rules|role\b|task\b)/i,
    /\[[^\]]*PROJ_ID[^\]]*\]/i,
    /:\s*$/
  ];

  const tooManyCategories = customCategories.length > 12;
  const tooLongCategory = customCategories.some(category => category.length > 100);
  const looksLikePrompt = customCategories.some(category => suspiciousPatterns.some(pattern => pattern.test(category)));

  if (tooManyCategories || tooLongCategory || looksLikePrompt) {
    return 'The Custom Categories box expects short labels, one per line. It looks like a full prompt was pasted there. Put instructions in Custom Prompt and keep categories as short headings.';
  }

  return null;
}

function extractProjectIdsFromPrompt(customPrompt) {
  if (typeof customPrompt !== 'string') {
    return [];
  }

  const matches = customPrompt.toUpperCase().match(/\bP\d{5,}\b/g) || [];
  return [...new Set(matches)];
}

function parseProjectIdsInput(value) {
  if (typeof value !== 'string') {
    return [];
  }

  const matches = value.toUpperCase().match(/\bP\d{5,}\b/g) || [];
  return [...new Set(matches)];
}

function formatProjectSelectionList(projects) {
  if (!projects || !projects.length) {
    return '<p class="project-selection-empty">None</p>';
  }

  return projects.map(project => {
    const name = project.project_name ? ` - ${escapeHtml(project.project_name)}` : '';
    return `
      <div class="project-selection-item">
        <div class="project-selection-item-main">
          <strong>${escapeHtml(project.project_id)}</strong>${name}
        </div>
        <div class="project-selection-item-meta">Rank ${project.rank} · ${project.risk_count} linked risk items</div>
      </div>
    `;
  }).join('');
}

const MAX_MANUAL_PROJECT_SELECTION = 15;

function buildProjectSelectionItems(projects, selectedProjectIds, maxSelectable) {
  if (!projects || !projects.length) {
    return '<p class="project-selection-empty">None</p>';
  }

  const selectedSet = new Set(selectedProjectIds);
  const isAtLimit = selectedProjectIds.length >= maxSelectable;

  return projects.map(project => {
    const isSelected = selectedSet.has(project.project_id);
    ...
    return `
      <button
        type="button"
        class="project-selection-item project-selection-toggle ${isSelected ? 'selected' : 'available'}"
      ...
      >
        ...
        <div class="project-selection-item-meta">Rank ${project.rank} · ${project.risk_count} linked risk items</div>
        <div class="project-selection-item-action">${actionLabel}</div>
      </button>
    `;
  }).join('');
}
...
function closeProjectSelectionModal() {
  if (projectSelectionResolve) {
    const resolve = projectSelectionResolve;
    projectSelectionResolve = null;
    resolve(null);
  }
...
function openProjectSelectionModal(selection, requestPayload) {
  const modal = document.getElementById('projectSelectionModal');
  const summary = document.getElementById('projectSelectionSummary');
  const notes = document.getElementById('projectSelectionNotes');
  const selectedList = document.getElementById('projectSelectionSelectedList');
  const discardedList = document.getElementById('projectSelectionDiscardedList');
  const selectedHeading = document.getElementById('projectSelectionSelectedHeading');
  const discardedHeading = document.getElementById('projectSelectionDiscardedHeading');
  const confirmBtn = document.getElementById('projectSelectionConfirmBtn');
  const maxSelectable = Math.min(selection.max_projects || MAX_MANUAL_PROJECT_SELECTION, MAX_MANUAL_PROJECT_SELECTION);
  const allProjects = [...(selection.selected_projects || []), ...(selection.discarded_projects || [])]
    .sort((left, right) => left.rank - right.rank);
  let selectedProjectIds = [...selection.selected_project_ids].slice(0, maxSelectable);

  const noteLines = [];
  if (selection.portfolio_too_large) {
    noteLines.push(`The full evidence set is larger than the briefing context limit, so the app would otherwise auto-select ${selection.max_projects} projects.`);
  }
  if (selection.prompt_requested_project_ids && selection.prompt_requested_project_ids.length) {
    noteLines.push(`The custom prompt references these project IDs: ${selection.prompt_requested_project_ids.join(', ')}.`);
  }
  if (selection.prompt_requested_project_ids_missing && selection.prompt_requested_project_ids_missing.length) {
    noteLines.push(`These prompt IDs were not found in the evidence: ${selection.prompt_requested_project_ids_missing.join(', ')}.`);
  }
  noteLines.push(`Click projects to add or remove them. You can include up to ${maxSelectable} projects.`);

  notes.innerHTML = noteLines.map(line => `<p>${escapeHtml(line)}</p>`).join('');
  ...
  return new Promise(resolve => {
    ...
    const renderLists = () => {
      const selectedSet = new Set(selectedProjectIds);
      const selectedProjects = allProjects.filter(project => selectedSet.has(project.project_id));
      const discardedProjects = allProjects.filter(project => !selectedSet.has(project.project_id));
      const selectedRiskCount = selectedProjects.reduce((total, project) => total + project.risk_count, 0);

      summary.textContent = `Selected ${selectedProjects.length} of ${selection.total_projects} projects (max ${maxSelectable}), covering ${selectedRiskCount} of ${selection.total_risks_available} linked risk items.`;
      selectedHeading.textContent = `Included Projects (${selectedProjects.length})`;
      discardedHeading.textContent = `Excluded Projects (${discardedProjects.length})`;
      selectedList.innerHTML = buildProjectSelectionItems(selectedProjects, selectedProjectIds, maxSelectable);
      discardedList.innerHTML = buildProjectSelectionItems(discardedProjects, selectedProjectIds, maxSelectable);
      confirmBtn.disabled = selectedProjectIds.length === 0;
      confirmBtn.textContent = `Continue with ${selectedProjectIds.length} Project${selectedProjectIds.length === 1 ? '' : 's'}`;

      modal.querySelectorAll('.project-selection-toggle').forEach(button => {
        button.onclick = () => {
          const projectId = button.dataset.projectId;
          const nextSelectedIds = selectedProjectIds.filter(currentId => currentId !== projectId);

          if (nextSelectedIds.length !== selectedProjectIds.length) {
            selectedProjectIds = nextSelectedIds;
            renderLists();
            return;
          }

          if (selectedProjectIds.length >= maxSelectable) {
            showStatus(`You can include up to ${maxSelectable} projects in the briefing.`, 'error');
            return;
          }

          selectedProjectIds = [...selectedProjectIds, projectId].sort((left, right) => {
            const leftProject = allProjects.find(project => project.project_id === left);
            const rightProject = allProjects.find(project => project.project_id === right);
            return (leftProject?.rank || 0) - (rightProject?.rank || 0);
          });
          renderLists();
        };
      });
    ...
    document.getElementById('projectSelectionCancelBtn').onclick = cancel;
    document.getElementById('projectSelectionCloseBtn').onclick = cancel;

    confirmBtn.onclick = () => {
      if (!selectedProjectIds.length) {
        showStatus('Select at least one project to continue.', 'error');
        return;
      }
...
async function maybeReviewProjectSelection(requestPayload) {
  const promptProjectIds = extractProjectIdsFromPrompt(requestPayload.custom_prompt);
  if (!promptProjectIds.length || requestPayload.selected_project_ids) {
  ...
  if (!response.ok || selection.error) {
    throw new Error(selection.error || 'Unable to preview project selection.');
  }

  if (selection.skip_preview) {
    return requestPayload;
  }

  return openProjectSelectionModal(selection, {
    ...requestPayload,
...
// Generate briefing
async function generateBriefing(customPrompt = null, selectedProjectIds = null, options = {}) {
  if (generating) return;

  const draftRequestPayload = buildBriefingRequest(customPrompt, selectedProjectIds, options);
  if (draftRequestPayload.mode === 'custom') {
    const categoryError = validateCustomCategories(draftRequestPayload.custom_categories);
    if (categoryError) {
      showStatus(categoryError, 'error');
      return;
    }
  }
  ...
  try {
    ...
    reviewedPayload = await maybeReviewProjectSelection(requestPayload);
    if (!reviewedPayload) {
      showStatus('Briefing generation canceled.', 'info');
      return;
    }
  } catch (error) {
    console.error('Project selection preview failed:', error);
    showStatus(`Project selection preview failed, proceeding without review: ${error.message}`, 'info');
    reviewedPayload = draftRequestPayload;
  }
  ...
  console.log('  - customPrompt:', promptOverride ? 'provided' : 'null');
  console.log('  - selectedProjectIds:', finalSelectedProjectIds);
  ...
  try {
    const response = await fetch('/api/briefing/generate', {
      ...
      body: JSON.stringify({
        ...
        n_paragraphs: nParagraphs,
        custom_categories: customCategories,
        custom_prompt: promptOverride,
        force_regenerate: forceRegenerate,
        regenerate_final_only: reviewedPayload.regenerate_final_only,
        selected_project_ids: finalSelectedProjectIds
      })
    });
    ...
    if (!response.ok || !contentType.includes('text/event-stream')) {
      ...
      let errorMessage = `Generation request failed (${response.status})`;
    ...
    console.log('DEBUG generateBriefing: Sent to server:', {
      ...
      n_paragraphs: nParagraphs,
      custom_categories: customCategories,
      custom_prompt: promptOverride ? '***PROMPT PROVIDED***' : null,
      force_regenerate: forceRegenerate,
      regenerate_final_only: reviewedPayload.regenerate_final_only,
      selected_project_ids: finalSelectedProjectIds
    });
...
async function initializePromptEditor() {
  ...
  regenerateBtn.onclick = () => {
    ...
    const mode = document.getElementById('briefingMode').value;
    const useFreshNews = document.getElementById('forceRegenerate').checked;
    console.log(`DEBUG: Regenerating with custom prompt, current mode=${mode}, useFreshNews=${useFreshNews}`);
    generateBriefing(customPrompt, null, {regenerateFinalOnly: !useFreshNews});
  };
}
